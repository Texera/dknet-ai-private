...
object AccessControlResource extends LazyLogging {
  ...
  private def checkComputingUnitAccess(
    ...
    val targetHost = recordedUri match {
      case Some(uri) =>
        // cu-ssh requests reach ttyd on its own port, not the CU's recorded one
        val host = if (requireSshPermission) s"${uri.split(":").head}:7681" else uri
        logger.info(s"Routing CU $cuidInt to host: $host")
        host
      case None =>
