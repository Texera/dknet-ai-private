...
object ComputingUnitManagingResource {
  ...
  private lazy val computingUnitEnvironmentVariables: Map[String, Any] =
    icebergEnvironmentVariables ++ Map(
      ...
      // Fork: source the CU's JWT secret from AuthConfig (deployment config) rather than
      // re-reading the ENV_AUTH_JWT_SECRET environment variable, which this deployment
      // does not set on the manager. Appended last so it wins over any looked-up value.
      Map(EnvironmentalVariable.ENV_AUTH_JWT_SECRET -> AuthConfig.jwtSecretKey)
