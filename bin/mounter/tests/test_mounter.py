...
def test_do_mount_asks_for_no_cache_by_default(mounter):
    ...
    assert "--cache" not in cmd  # geesefs's own default, kept by an unconfigured deployment


def test_do_mount_caches_by_repository_and_commit_when_a_cache_root_is_set(mounter, tmp_path, monkeypatch):
    (tmp_path / "cache").mkdir()  # the chart mounts CACHE_ROOT as a volume, so it is there
    ...
    # Keyed by repo and commit, never by cuid: a commit is immutable, so a second unit on this
    # node reuses the first one's cache instead of starting cold.
