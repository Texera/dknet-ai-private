...
def do_mount(cuid, repo, commit, jwt, file_service_base):
    ...
    if CACHE_ROOT:
        ...
        # mounts it as a volume, so if it is missing the volume is missing -- and creating it
        # would put the cache in the container's own writable layer, where it is lost on
